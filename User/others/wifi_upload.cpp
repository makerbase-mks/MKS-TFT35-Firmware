...
void ResetWiFiForUpload(int begin_or_end)
{
	...
	if(begin_or_end == 0)
	{
		HAL_GPIO_WritePin(GPIOC,GPIO_Pin_7,GPIO_PIN_RESET); //update mode
	}
	else
	{
		HAL_GPIO_WritePin(GPIOC,GPIO_Pin_7,GPIO_PIN_SET); //boot mode	
		GPIO_InitStructure.Speed = GPIO_SPEED_FREQ_HIGH;
		GPIO_InitStructure.Pin = GPIO_Pin_7;//PF2先作为输入，升级时候作为输出
